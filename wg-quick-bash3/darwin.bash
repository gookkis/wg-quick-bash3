...
#
# Copyright (C) 2024-2034 Heri Kiswanto <heryoulie@gookkis>. All Rights Reserved.
#
...
collect_new_service_dns() {
    ...
    for service in "${SERVICE_DNS[@]}"; do
        if ! [[ " ${found_services[*]} " == *" $service "* ]]; then
            SERVICE_DNS=( "${SERVICE_DNS[@]/$service}" )
            SERVICE_DNS_SEARCH=( "${SERVICE_DNS_SEARCH[@]/$service}" )
        fi
    done
}
...
del_dns() {
    ...
    for service in "${SERVICE_DNS[@]}"; do
        ...
        while read -r response; do
            ...
            cmd networksetup -setdnsservers "$service" "${service#*:}" || true
            cmd networksetup -setsearchdomains "$service" "${service#*:}" || true
        )
    done
}
...
save_config() {
    ...
    while read -r address; do
        if [[ $address =~ inet6?\ ([^ ]+) ]]; then
            new_config+="Address = ${BASH_REMATCH[1]}"$'\n'
        fi
    ...
    old_umask=`umask`
    umask 077
    current_config=`wg showconf "$REAL_INTERFACE"`
...
cmd_up() {
    ...
    set_mtu
    up_if
    for i in `wg show "$REAL_INTERFACE" allowed-ips | awk '{print $2}' | sort -nr -k 2 -t /`; do
        add_route "$i"
    done
    [[ $AUTO_ROUTE4 -eq 1 || $AUTO_ROUTE6 -eq 1 ]] && set_endpoint_direct_route
    [[ ${#DNS[@]} -gt 0 ]] && set_dns
